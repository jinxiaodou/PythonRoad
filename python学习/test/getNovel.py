...
def parseArticle(bs):
	...
	title=chapter[0].string.split('（')[0] #去多余文字
	title='第'+title.split(' ')[0]+'章 '+title.split(' ')[-1]# 添加第 章，以便生成目录
	article=bs.select('div#content')
	articleStr=""
	for item in article[0].children:
		if item is not None and item.name != 'br' and item.name != 'script' and item.name != 'div':	#去除br
			articleStr+='\n'+item
	# print(articleStr)
	return title+'\n\n'+articleStr
...
def run():
	...
	print('从第'+str(beginId)+'章开始下载《'+bookName+'》')
	for chapter in chapters[beginId:]:
		...
		sys.stdout.write('已下载第%d章：%.2f%%' % (current,progress*100) + '\r')
		sys.stdout.flush()
